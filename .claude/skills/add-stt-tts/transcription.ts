...
const _env = readEnvFile(['WHISPER_BIN', 'WHISPER_MODEL', 'WHISPER_LANG']);
const WHISPER_BIN = _env.WHISPER_BIN || 'whisper-cli';
const WHISPER_MODEL =
  _env.WHISPER_MODEL ||
  path.join(process.cwd(), 'data', 'models', 'ggml-base.bin');
const WHISPER_LANG = _env.WHISPER_LANG || '';
...
async function transcribeWithWhisperCpp(
  ...
  try {
    ...
    const langArgs = WHISPER_LANG ? ['-l', WHISPER_LANG] : [];
    const { stdout } = await execFileAsync(
      WHISPER_BIN,
      ['-m', WHISPER_MODEL, '-f', tmpWav, '--no-timestamps', '-nt', ...langArgs],
      { timeout: 60_000 },
    );
